	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed frag(v2f i) : SV_Target
			{
				...
				if(h.x<pos.y) return 0;//air
				
				int d = h.x-pos.y;
				if(h.y>2&&h.y<8&&(h.x-4<pos.y))
				{
					if(h.y==4)
					{
						return float(d==0?19:d<3?18:17)/255;
					}
					if(h.y==3)
					{
						return float(d<3?27:17)/255;
					}
					if(h.y==5)
					{
						return float(d<3?18:17)/255;
					}
					if(h.y==7)
					{
						return float(d<2?25:17)/255;
					}
					if(h.y==6)
					{
						return float(d<2?26:17)/255;
					}
				}
				...
				if(h.y==8&&(h.x-6<pos.y)) return float(d<3?27:28)/255;

				if(h.y==9&&(h.x-4<pos.y)) return float(28)/255;
